...
extension Node {
    ...
    /// 進這個容器盤點頁的那一列。**永遠出現，不設任何條件。**
    ///
    /// 試過兩個比較「聰明」的版本，兩個都會生出到不了的畫面，原因是同一個：
    /// §2.1 拒絕區分容器與物件，所以任何條件都等於偷渡那個區分回來，
    /// 而被條件擋掉的節點，它們的盤點頁就沒有入口了。
    ///
    /// - 用「有沒有子節點」：被搬空的包，歸屬在它底下的東西仍以 `✗` 列在它的盤點頁上，
    ///   而那是使用者要把它們標回來的地方
    /// - 用「盤點頁列不列得出東西」：剛建好的空容器兩邊都是空的，於是它的盤點頁到不了 ——
    ///   而那一頁正是把第一件東西放進去的入口（§4.2d 的「加入第一件」）
    ///
    /// N 用遞迴件數（§3.1），與首頁、盤點頁標頭同一個數字，所以空的時候讀作「裡面有 0 件」。
    /// 見 `docs/SPEC.md` §4.4c。
