...
**說清楚一件會讓人猶豫的事：在垂直這個軸上，16e 反而比較接近 Figma**
（52.3 對 52），iPhone 16 是 64.3。但那是巧合，不是 16e 的優點 ——
Figma 那個 52 只是畫給人看的狀態列佔位，程式碼從來沒有實作它。
拿一台「剛好對上佔位值」的機器當基準，等於把巧合當成契約：
換一支 iOS 版本、換一台裝置，安全區一變它就不再成立，而且會被讀成版型壞了。
換基準機換到的是**版面寬度與可視高度**的精確 —— 那兩個才是 Figma frame 真的在描述的東西。
